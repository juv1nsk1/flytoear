...
export const createServiceKLender = (config:any) => {
  ...
  servicesLender.get("/loan", async (req: Request, res: Response) => {
    ...
    return res.status(200).json({ collateralAmount, interestAmount, principal, startTimestamp , isApproved: output.isApproved });
  });
